...
export const fetchSalesOrder = createAsyncThunk<
  { items: SalesOrder[]; fetchedAt: string },
  { force?: boolean } | void,
  { state: any; rejectValue: string }
>(
  "saleOrder/fetchSalesOrder",
  async (_arg, { rejectWithValue }) => {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`Sales Order API failed (${res.status}): ${t.slice(0, 120)}`);
      }

      const ct = res.headers.get("content-type") || "";
      if (!ct.includes("application/json")) {
        const text = await res.text();
        throw new Error(`Sales Order Data Non-JSON response: ${text.slice(0, 120)}`);
      }

      const json = await res.json();
      const rows = unwrapRows<SalesOrder>(json, "Sales Order Data");

      return { items: rows, fetchedAt: new Date().toISOString() };
    } catch (e: any) {
      return rejectWithValue(e?.message || "Failed to fetch sales order data");
    }
  },
  {
    condition: (arg, { getState }) => {
      ...
      const s: SalesOrderState = state.saleOrder;
...
export const { clearSalesOrder } = salesOrderSlice.actions;
